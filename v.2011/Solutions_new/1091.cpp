...
const int PLUS_COUNT=9;
const int PLUS_NUMBERS[PLUS_COUNT]={2,3,5,7,11,13,17,19,23};
const int MINUS_COUNT=6;
const int MINUS_NUMBERS[MINUS_COUNT]={6,10,14,15,21,22};
const int MAX_S=50;
...
public:
	...
	long long getC(int n,int k); // получение числа сочетаний из n по k
...
long long calculator::getC(int n,int k) {
...
int main() {
  int s,k,ans=0;
  int *count=new int[MAX_S];
  calculator c;
  memset(count,0,MAX_S*sizeof(int));
  cin>>k>>s;
  for(int i=0;i<s;i++)
    count[i]=s/(i+2);
  for(int i=0;i<PLUS_COUNT;i++)
    if(count[PLUS_NUMBERS[i]-2]>=k)
      ans+=c.getC(count[PLUS_NUMBERS[i]-2],k);
  for(int i=0;i<MINUS_COUNT;i++)
    if(count[MINUS_NUMBERS[i]-2]>=k)
      ans-=c.getC(count[MINUS_NUMBERS[i]-2],k);
  if(ans>MAX_ANS)
		cout<<MAX_ANS;
  else
		cout<<ans;
