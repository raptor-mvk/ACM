...
public:
	...
	// считывание слова в направлении dir(вместе со стандартными разделителями,
	// если readln = true), функция checkFunc должна возвращать
...
int main() {
  ...
  r->fill();
	r->getNext(&str,&isCapitalLetter,'f',false);
	cout<<str;
  while(r->getNext(&str,&isDelim,'f',false)) {
		cout<<str[0]<<str.substr(1).process(&lowerCase);
		if(r->getNext(&str,&isCapitalLetter,'f',false))
			cout<<str;
		else
			break;
	}
