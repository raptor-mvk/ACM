...
int main() {
	int shifts[DIRECTIONS_COUNT],len;
	reader *r;
	char data[MAX_DATA_SIZE];
	bool destroyed=false;
	memset(shifts,0,DIRECTIONS_COUNT*sizeof(int));
	...
	cout<<setprecision(10)<<fixed<<((double)shifts[0]+(double)(shifts[2]-
		shifts[3])/sqrt(2.0))<<' '<<((double)shifts[1]+(double)(shifts[2]+
		shifts[3])/sqrt(2.0));
}
