...
int main() {
	...
	if(str.getLength()<2)
		...
		first=huge(str.substrl(0,str.getLength()/2));
		firstOdd=huge(str.substrl(0,str.getLength()/2+1));
		if(str.substrl(0,str.getLength()/2).reverse()<
			str.substr(str.getLength()/2+str.getLength()%2)) {
				if(str.getLength()%2) {
					firstOdd+=1;
					cout<<((fstring)firstOdd)<<((fstring)firstOdd).trimEnd(1).reverse();
				}
				else {
					first+=1;
					cout<<first<<((fstring)first).reverse();
				}
		}
		else {
			if(str.getLength()%2)
				cout<<first<<str[str.getLength()/2]<<((fstring)first).reverse();
			else
				cout<<first<<((fstring)first).reverse();
		}
	}
