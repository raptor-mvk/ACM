...
const int MAX_DATA_SIZE=10001;
const int TRAM_WORD_LENGTH=4;
const char TRAM_WORD[TRAM_WORD_LENGTH+1]="tram";
const int TROLLEYBUS_WORD_LENGTH=10;
const char TROLLEYBUS_WORD[TROLLEYBUS_WORD_LENGTH+1]="trolleybus";
...
class fstring {
public:
	static const int MAX_LENGTH=10001; // максимальная длина строки
...
int main() {
	int tram=0,trolleybus=0;
	char *data;
	fstring str;
	fstring tramStr((char*)TRAM_WORD,TRAM_WORD_LENGTH);
	fstring trolleybusStr((char*)TROLLEYBUS_WORD,TROLLEYBUS_WORD_LENGTH);
	reader *r;
	data=new char[MAX_DATA_SIZE];
	r=new reader(cin,data,MAX_DATA_SIZE);
	r->fill();
	while(r->getNext(&str,&isLowercaseLetter,'f',false)) {
		if(r->getNext(&str,&isPunctuationOrDelim,'f',false)) {
			if(str==tramStr)
				tram++;
			else
				if(str==trolleybusStr)
					trolleybus++;
		}
	}
	if(tram>trolleybus)
