...
public:
	...
	template <typename type> bool getNext(type *number,char dir,
		bool readln=false);

	// считывание слова в направлении dir(вместе с разделителями, если readln =
	// true), функция checkFunc должна возвращать true для разделителя
	fstring getNext(bool(*checkFunc)(char symbol),char dir,bool readln=false);
...
int main() {
	...
	map=new char[n*n];
	for(int i=0;i<n;i++) {
		str=r->getNext(&checkFunc,'f',true);
		for(int j=0;j<n;j++)
			map[i*n+j]=(str[j]==EMPTY);
	}
	while(map[n*n-1]==1) {
		if(map[0]==1) {
			queue.push(0);
		...
		while(start<queue.getLength()) {
			if((queue[start]%n)&&(map[queue[start]-1]==1)) {
				queue.push(queue[start]-1);
				map[queue[start]-1]=2;
			}
			else
				if(!(queue[start]%n)||!map[queue[start]-1])
					square++;
			if((queue[start]%n<n-1)&&(map[queue[start]+1]==1)) {
				queue.push(queue[start]+1);
				map[queue[start]+1]=2;
			}
			else
				if((queue[start]%n>=n-1)||!map[queue[start]+1])
					square++;
			if((queue[start]>=n)&&(map[queue[start]-n]==1)) {
				queue.push(queue[start]-n);
				map[queue[start]-n]=2;
			}
			else
				if((queue[start]<n)||!map[queue[start]-n])
					square++;
			if((queue[start]<n*(n-1))&&(map[queue[start]+n]==1)) {
				queue.push(queue[start]+n);
				map[queue[start]+n]=2;
			}
			else
				if((queue[start]>=n*(n-1))||!map[queue[start]+n])
					square++;
			start++;
		}
	}
	cout<<((square-4)*WALL_SQUARE);
}
